...
namespace Portal.Repository.Repositories
{
    public class Repository<TEntity> : BaseRepository, IRepository<TEntity> where TEntity : class
    {
        ...
        public async Task<ListResponse<TEntity>> GetAll2<TEntity>(int page, int pageSize) where TEntity : class
        {
            var tableName = GetTableName();
    ...
    {
        var result = new ListResponse<TEntity>();
        var page = 1;

        while (true)
        {
            var items = GetAll2<TEntity>(page, pageSize).Result;
